...
vec4 useLight(Light light, vec4 surfacePostion, vec4 normal_transformed, vec3 normal, Material material )
{
    ...
    if(light.light_position.w == 1.0)
    {
		...
		if(light_to_surface_angle <= light.outer_angle && light_to_surface_angle >= light.inner_angle){
			smooth_coefficient = 1.0 - smoothstep(light.inner_angle, light.outer_angle, light_to_surface_angle);
		}
