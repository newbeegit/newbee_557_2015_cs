...
string GLSpotLightSource::_glsl_names[4] = { "cone_angle","inner_angle","outer_angle", "cone_direction" };
#endif
...
bool GLSpotLightSource::addVariablesToProgram(GLuint program, int variable_index)
{
	...
	_inner_angleIdx = glGetUniformLocation(program, GetVariableName(_glsl_object, _glsl_names[1], variable_index).c_str());
	checkUniform(_inner_angleIdx, GetVariableName(_glsl_object, _glsl_names[1], variable_index));

	_outer_angleIdx = glGetUniformLocation(program, GetVariableName(_glsl_object, _glsl_names[2], variable_index).c_str());
	checkUniform(_outer_angleIdx, GetVariableName(_glsl_object, _glsl_names[2], variable_index));
	...
	glUniform1f(_inner_angleIdx, _inner_angle);
	glUniform1f(_outer_angleIdx, _outer_angle);
